...
def procesar_datos(df):
    ...
    try:
        # 1. Preparación de datos: Convertir la columna de fecha a formato datetime
        df['Fecha Última Actuación'] = pd.to_datetime(df['Fecha Última Actuación'], errors='coerce')
        
        # Eliminar filas con fechas no válidas
        df.dropna(subset=['Fecha Última Actuación'], inplace=True)
        
        # 2. Obtener lista única de actuaciones
        lista_actuaciones = df['Última Actuación'].unique().tolist()
        lista_actuaciones.sort() # Ordenar alfabéticamente
        
        # 3. Clasificación por Fecha (Reciente a Antigua)
        df_clasificado_fecha = df.sort_values(by='Fecha Última Actuación', ascending=False)
        ...
        # Filtrar los procesos: Usamos .dt.floor('D') para estandarizar la columna de fechas a medianoche.
        procesos_atrasados = df[df['Fecha Última Actuación'].dt.floor('D') < fecha_limite]
        
        if not procesos_atrasados.empty:
            # Calcular la antigüedad en días y ordenar
            procesos_atrasados['Días de Antigüedad'] = (fecha_actual - procesos_atrasados['Fecha Última Actuación']).dt.days
        ...
        st.error(f"Error: No se encontró la columna clave requerida. Asegúrate de que tu archivo contenga las columnas 'Caso Noticia', 'Fecha Última Actuación' y 'Última Actuación'. Detalles: {e}")
...
uploaded_file = st.file_uploader(
    "📂 Sube tu archivo CSV de procesos (Ej: carga_ficticia_111.csv)", 
    type="csv"
)
...
if uploaded_file is not None:
    ...
    if lista_actuaciones is not None:
        ...
        st.sidebar.markdown("Selecciona una o varias actuaciones para ver solo esos procesos en la tabla de clasificación.")
        
        # El usuario selecciona las actuaciones
        actuaciones_seleccionadas = st.sidebar.multiselect(
            'Elige las "Últimas Actuaciones" que deseas ver:',
            options=lista_actuaciones,
            default=lista_actuaciones # Por defecto, muestra todas
        )
        ...
        st.markdown("Tabla que muestra los procesos filtrados y ordenados por fecha de actuación (más reciente primero).")
        
        # Aplicar el filtro de las actuaciones seleccionadas
        if actuaciones_seleccionadas:
            df_filtrado_clasificado = df_clasificado_fecha[df_clasificado_fecha['Última Actuación'].isin(actuaciones_seleccionadas)]
        else:
            # Si no selecciona ninguna, mostramos la tabla vacía o un mensaje
            df_filtrado_clasificado = pd.DataFrame()
            st.warning("Selecciona al menos una 'Última Actuación' en el menú de la izquierda para ver los resultados.")
        ...
        if not df_filtrado_clasificado.empty:
            ...
            st.dataframe(df_filtrado_clasificado[['Caso Noticia', 'Última Actuación', 'Fecha Última Actuación']], use_container_width=True)
